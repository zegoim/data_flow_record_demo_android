...
class WhiteboardContainer : FrameLayout {
    ...
    fun getWhiteboardViewHolders(whiteboardID: Long): Sequence<View> {
        return children.filter { childView ->
            val zegoWhiteboardView = childView as ZegoWhiteboardViewHolder
            zegoWhiteboardView.hasWhiteboardID(whiteboardID)
        }
    }
    ...
    fun removeWhiteboardViewHolder(whiteboardID: Long) {
        getWhiteboardViewHolders(whiteboardID).forEach {
            removeView(it)
        }
    }
    ...
    fun resize(activity: Activity) {
        ...
        calcSize(orientation, deviceSize.x, deviceSize.y)
    }
    ...
    override fun onLayout(changed: Boolean, left: Int, top: Int, right: Int, bottom: Int) {
        ...
        Log.d(TAG, "onLayout:$left,$top,${width},${height}")
    }

    private fun calcSize(orientation: Int, deviceWidth: Int, deviceHeight: Int) {
        Log.i(
            TAG,
        ...
        val topHeight = ZegoUtil.dip2px(context, 98f)
        val settingHeight = ZegoUtil.dip2px(context, 40f)
        val bottomHeight = ZegoUtil.dip2px(context, 20f)
        if (orientation == Configuration.ORIENTATION_PORTRAIT
        ) {
            selfWidth = deviceWidth
            selfHeight = (selfWidth / (wbAspectWidth.toFloat() / wbAspectHeight.toFloat())).toInt()
...
//            parentHeight = (selfHeight + topHeight + bottomHeight +settingHeight )
    ...
    override fun onConfigurationChanged(newConfig: Configuration) {
        ...
        val deviceWidth = ZegoUtil.dip2px(context, newConfig.screenWidthDp.toFloat())
        val deviceHeight = ZegoUtil.dip2px(context, newConfig.screenHeightDp.toFloat())
    ...
    fun createPureWhiteboard(requestResult: (Int, ZegoWhiteboardViewHolder) -> Unit) {
        ...
        val userName = "测试"
        val whiteboardName = "${userName}创建的白板 1"
    ...
    fun createFileWhiteBoardView(
        ...
        if (holder != null) {
            ...
            viewHolder.createDocsAndWhiteBoardView(fileID, Size(width, height)) { errorCode ->
                if (errorCode == 0) {
                    Log.i(TAG, "viewHolder.currentWhiteboardID: " + viewHolder.currentWhiteboardID)
    ...
    fun onReceiveWhiteboardView(
        ...
        if (fileID.isNotEmpty()) {
            ...
            holder.onReceiveFileWhiteboard(Size(model.aspectWidth, model.aspectHeight), zegoWhiteboardView)
            { errorCode, _ ->
